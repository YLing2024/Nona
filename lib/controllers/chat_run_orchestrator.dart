    ...
    try {
      ...
      if (text.trim().isNotEmpty) {
        ...
        if (kbChunks.isNotEmpty) {
          final kbPrompt = localized.orchestratorKbPrompt(
            kbChunks
                .map((c) => '- ${c.docName}#${c.chunkId ?? ''} ${c.text}')
                .join('\n'),
          );
          options = options.copyWith(
            systemPrompt: [
        ...
        if (query.isNotEmpty) {
          ...
          if (outcome.ok) {
            if (outcome.items.isNotEmpty) {
              final searchPrompt =
                  WebSearchService.resultsToPrompt(outcome.items);
              options = options.copyWith(
                systemPrompt: [
                  searchPrompt,
                  options.systemPrompt,
