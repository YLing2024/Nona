...
class ChatMessage {
  ...
  /// 引用出处（B-03，citations_json 列；搜索/知识库来源）。
  List<CitationSource> citations;

  /// 消息分段权威数据（B-06，parts_json 列；text/reasoning/tool_call/
  /// tool_result 分段，null = 旧消息读 content 兼容）。
  String? partsJson;

  /// 工具执行步骤（F-04，tool_steps_json 列，重进会话可回看）。
  String? toolStepsJson;

  /// 流式生成状态（B-06：streaming 期间为 'streaming'，终态置空）。
  ...
  factory ChatMessage.fromJson(Map<String, dynamic> json) {
    return ChatMessage(
      ...
      citations: (json['citations'] as List<dynamic>? ?? [])
          ...
          .toList(),
      partsJson: json['partsJson'] as String?,
      toolStepsJson: json['toolStepsJson'] as String?,
      streamingState: json['streamingState'] as String?,
    );
  }
  ...
  Map<String, dynamic> toJson() => {
    ...
    if (partsJson != null) 'partsJson': partsJson,
    if (toolStepsJson != null) 'toolStepsJson': toolStepsJson,
    if (streamingState != null) 'streamingState': streamingState,
  };
}
