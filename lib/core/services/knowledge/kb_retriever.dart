...
class KbRetriever {
  ...
  Future<List<_Hit>> _keywordSearch(
    ...
    final params = <Variable>[
      for (final b in bigrams) Variable.withString(b),
    ];
    final libFilter = _libFilter();
    params.addAll([
      for (final id in libraryIds ?? const <String>[]) Variable.withString(id),
    ]);
    try {
      final rows = await db
          .customSelect(
            ...
            variables: [...params, Variable.withInt(topK)],
          )
          .get();
      return [
        for (final r in rows)
          _Hit(
            ...
            docName: r.data['doc_name'] as String,
            text: r.data['text'] as String,
            score: (r.data['score'] as int? ?? 0) / bigrams.length,
          ),
      ];
    } catch (_) {
      return const [];
  ...
  Future<List<_Hit>> _vectorSearch({required int topK}) async {
    final rows = await db
        .customSelect(
          ...
          variables: [
            for (final id in libraryIds ?? const <String>[])
