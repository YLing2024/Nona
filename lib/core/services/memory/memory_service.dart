...
class Memory {
  ...
  /// 从 drift 行构造（v2 字段解析：tags/priority/use_count/last_used_at/history_json）。
  factory Memory.fromRow(MemoryRow r) {
    List<String> parseTags() {
      try {
        final v = jsonDecode(r.tagsJson);
    ...
    List<MemoryVersion> parseHistory() {
      try {
        final v = jsonDecode(r.historyJson);
        if (v is List) {
          return [
            for (final e in v)
...
class MemorySpaceConfig {
  ...
  factory MemorySpaceConfig.fromRow(MemorySpaceRow r) => MemorySpaceConfig(
    scope: MemoryScope.values.firstWhere(
      (s) => s.name == r.scope,
      orElse: () => MemoryScope.global,
    ),
    scopeRef: r.scopeRef,
    maxItems: r.maxItems,
    maxInjectTokens: r.maxInjectTokens,
    maxItemChars: r.maxItemChars,
    extractionInterval: r.extractionInterval,
  );
...
class MemoryService {
    ...
    try {
        ...
        if (_similar(trimmed, m.content) >= 0.5) {
          ...
          await (db.update(db.memories)..where((t) => t.id.equals(m.id)))
            ...
            MemoriesCompanion(
              ...
              priority: Value(mergedPriority.name),
              historyJson: Value(jsonEncode([for (final h in history) h.toJson()])),
      ...
      await db.into(db.memories).insert(
        MemoriesCompanion.insert(
          ...
          historyJson: Value(jsonEncode(const [])),
  ...
  Future<void> update(
    ...
    try {
      final rows =
          await (db.select(db.memories)..where((t) => t.id.equals(id))).get();
      final old = rows.isEmpty ? null : Memory.fromRow(rows.first);
      final history = [
        ...?old?.history,
      ...
      await (db.update(db.memories)..where((t) => t.id.equals(id)))
        ...
        MemoriesCompanion(
          content: Value(trimmed),
          category: Value(category),
          tagsJson: Value(jsonEncode(tags ?? old?.tags ?? const <String>[])),
          priority: Value(
  ...
  /// 读取某作用域的空间预算（缺失时返回默认值）。
  Future<MemorySpaceConfig> spaceConfig({
    required MemoryScope scope,
    String scopeRef = '',
  }) async {
    final db = await _db;
    if (db == null) {
      return MemorySpaceConfig(scope: scope, scopeRef: scopeRef);
    }
    try {
      final row = await (db.select(db.memorySpaces)
            ..where((t) => t.scope.equals(scope.name) & t.scopeRef.equals(scopeRef)))
          .getSingleOrNull();
      if (row != null) return MemorySpaceConfig.fromRow(row);
    } catch (_) {}
    return MemorySpaceConfig(scope: scope, scopeRef: scopeRef);
  }

  /// 保存作用域空间预算。
  Future<void> saveSpaceConfig(MemorySpaceConfig config) async {
    final db = await _db;
    if (db == null) return;
    try {
      final id = '${config.scope.name}|${config.scopeRef}';
      await db.into(db.memorySpaces).insertOnConflictUpdate(
        MemorySpacesCompanion.insert(
          id: id,
          scope: config.scope.name,
          scopeRef: Value(config.scopeRef),
          maxItems: Value(config.maxItems),
          maxInjectTokens: Value(config.maxInjectTokens),
          maxItemChars: Value(config.maxItemChars),
          extractionInterval: Value(config.extractionInterval),
        ),
      );
    } catch (_) {}
  }
