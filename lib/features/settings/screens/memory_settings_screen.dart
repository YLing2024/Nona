...
class _MemorySettingsScreenState extends State<MemorySettingsScreen> {
  ...
  Future<void> _load() async {
    ...
    if (!mounted) return;
    setState(() {
      _enabled = s.memoryEnabled;
      _maxItems = s.memoryMaxItems;
      _maxInjectTokens = s.memoryMaxInjectTokens;
      _maxItemChars = s.memoryMaxItemChars;
      _extractionInterval = s.memoryExtractionInterval;
      _loading = false;
    });
  }
  ...
  Future<void> _save() async {
    ...
    await _settings.save(
      s.copyWith(
        memoryEnabled: _enabled,
        memoryMaxItems: _maxItems,
        memoryMaxInjectTokens: _maxInjectTokens,
        memoryMaxItemChars: _maxItemChars,
        memoryExtractionInterval: _extractionInterval,
      ),
    );
    if (mounted) {
