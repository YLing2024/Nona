...
Future<void> main() async {
  ...
  await runZonedGuarded(() async {
    ...
    if (!kIsWeb) {
      ...
      if (!primary) {
        return; // 已有实例运行：本实例直接退出
      }
      unawaited(DesktopLauncher.registerFileAssociation());
    }
    ...
    // H-02/H-03：显示设置（字体/密度/字号）
    fontFamilyNotifier.value = settings.fontFamily;
    uiDensityNotifier.value = settings.uiDensity;
    chatFontScaleNotifier.value = settings.chatFontScale;
    ...
    // J-04：恢复持久化的网络日志改为非阻塞（首帧后执行，不拖慢冷启动）
    unawaited(NetworkLogService.instance.init());
    ...
    unawaited(RestoreService.convergeOnStartup());
    ...
    // J-04：首帧耗时打点（profile 模式可观测）
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Logger.debug(
        'startup',
        'first frame at ${stopwatch.elapsedMilliseconds}ms',
      );
    });
  }, (e, s) {
                        ...
                        builder: (context, locale, _) {
                          return ValueListenableBuilder<String>(
                            ...
                            builder: (context, fontFamily, _) {
                              return ValueListenableBuilder<String>(
                                valueListenable: uiDensityNotifier,
                                builder: (context, density, _) {
                                  return ValueListenableBuilder<double>(
                                    valueListenable: chatFontScaleNotifier,
