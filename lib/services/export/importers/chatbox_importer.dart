...
/// Chatbox 备份解析：`{version, exportTime, conversations: [{id, name,
/// updatedAt, messages: [{id, role, content, time, dataType}]}]}`。
class ChatboxImporter {
  static bool matches(Map<String, dynamic> data) => data['conversations'] is List;
        ...
        result.add(
          ChatSession(
            ...
            title: (c['name'] as String? ?? '').trim().isEmpty
                ...
                : c['name'] as String,
            messages: messages,
            createdAt: parseImportTime(c['updatedAt'] ?? c['createdAt']) ?? now,
  ...
  /// 解析单条 Chatbox 消息：content 可为字符串或 [{type, text, url}] 数组。
  static ChatMessage? messageFrom(Map<String, dynamic> m) {
    ...
    if (raw is String) {
      ...
      for (final part in raw) {
        ...
        if (type == 'image') {
          final url = (part['url'] ?? part['local_url'] ?? part['data_url']) as String?;
          if (url != null) images.add(imageFromUrl(url));
        }
      }
    }
    if (sb.isEmpty && images.isEmpty) return null;
    return ChatMessage(
      role: role,
