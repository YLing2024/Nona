...
import 'package:nona_chat/core/services/knowledge/embedding_provider.dart';
  ...
  group('F4-2 混合检索', () {
    ...
    test('mock embedding 服务参与检索', () async {
      ...
      await db!.transaction(() async {
        final rows = await db
            .customSelect('SELECT id, text FROM kb_chunks')
            .get();
        for (final r in rows) {
          await db.customStatement(
            ...
            [
              r.data['id'],
              Float32Codec.encode(vectors[r.data['text']] ?? [0.0, 0.0, 0.0]),
            ],
          );
        }
      });
